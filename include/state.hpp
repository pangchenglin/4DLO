...
public:
    ...
    StatesGroup one_point_propag(const double timestamp,const StatesGroup &state_input);
...
StatesGroup State_Process::one_point_propag(const double timestamp,const StatesGroup &state_input){
    cov_acc = Eye3d * cov_acc_scale;
    cov_gyr = Eye3d * cov_gyr_scale;

    double dt = timestamp - time_last_scan;
    time_last_scan = timestamp;
    MD(DIM_STATE,DIM_STATE) F_x,  cov_w;
    StatesGroup state_output;
...
// void State_Process::point_by_point_propag(const RTVPointCloud::Ptr &meas,const double &timestamp,std::vector<StatesGroup> &PoseVector ){
//   cov_acc = Eye3d * cov_acc_scale;
//   cov_gyr = Eye3d * cov_gyr_scale;
...
//     time_last_scan = timestamp;
//   }else{
//     for(auto point : meas->points){
//       dt = point.time + timestamp  - time_last_scan;
//       StatesGroup state_input;
//       M3D Exp_f = Exp(state_input.bias_g,dt);
//       F_x.setIdentity();
//       cov_w.setZero();
//       F_x.block<3, 3>(0, 0) = Exp(state_input.bias_g, -dt);
...
//       PoseVector.push_back(state_input);
//     }
//   }
// }
