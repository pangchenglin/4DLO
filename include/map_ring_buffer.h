...
public:
    ...
    void addMeas(const Meas &meas, const double &t)
    {
        ...
        if (static_cast<int>(measMap_.size()) > size) {
   ...
   bool getLastMeas(Meas &lastMeas)
    ...
    bool getMeas(double &time,Meas &Measure){
        if(!measMap_.empty()){
           itMeas_ = measMap_.find(time);
            if(itMeas_ != measMap_.end())
            {
                Measure = itMeas_->second;
                return true;
            }else{
                return false;
            }
    ...
    bool measUpdate(double &time,Meas &Measure){
        if(!measMap_.empty()){
           itMeas_ = measMap_.find(time);
            if(itMeas_ != measMap_.end())
            {
                itMeas_->second = Measure;
                return true;
            }else{
                return false;
            }
