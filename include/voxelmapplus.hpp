...
public:
  ...
  void  init_plane(const std::vector<pointWithCov> &points, Plane *plane) {
    ...
    // std::cout<<"plane->points_size:"<<points.size()<<std::endl;
    plane->radius = 0;
    for (auto pv : points) {
      plane->covariance += pv.point * pv.point.transpose();
      plane->center += pv.point;
      // std::cout<<"point:"<<pv.point.transpose()<<std::endl;
    }
    ...
    // std::cout<<"cov:"<<plane->covariance<<std::endl;
    // std::cout<<"evalsReal:"<<evalsReal.transpose()<<std::endl;

    Eigen::Matrix3f::Index evalsMin, evalsMax;
    evalsReal.rowwise().sum().minCoeff(&evalsMin);
    evalsReal.rowwise().sum().maxCoeff(&evalsMax);
    int evalsMid = 3 - evalsMin - evalsMax;
    // std::cout<<"evalsMin:"<<evalsMin<<",evalsMid:"<<evalsMid<<",evalsMax:"<<evalsMax<<std::endl;
...
void updateVoxelMap(const std::vector<pointWithCov> &input_points,
  ...
  for (uint i = 0; i < plsize; i++) {
    const pointWithCov p_v = input_points[i];
    // if(p_v.point.x() == 0 && p_v.point.y() == 0 &&p_v.point.z() == 0 )
    // {
    ...
    if (range_dis <= radius_k * plane.radius) {
      ...
      if (dis_to_plane < sigma_num * sqrt(sigma_l)) {
        ...
        if (this_prob > prob) {
          ...
          single_ptpl.point_raw = pv.point_raw;
...
//   }
// }
bool BuildSingleResidual(const unordered_map<VOXEL_LOC, OctoTree *> &voxel_map,
const double voxel_size,const double sigma_num,const int max_layer,const pointWithCov &pv,ptpl &ptpl_res)
{
  float loc_xyz[3];
  for (int j = 0; j < 3; j++) {
    loc_xyz[j] = pv.point_world[j] / voxel_size;
...
void pubVoxelMap(const std::unordered_map<VOXEL_LOC, OctoTree *> &voxel_map,
  ...
  for (size_t i = 0; i < pub_plane_list.size(); i++) {
    Eigen::Vector3d plane_cov = pub_plane_list[i].plane_cov.block<3, 3>(0, 0).diagonal();
      ...
      for (uint i = 0; i < 8; i++) {
        if (iter->second->leaves_[i] != nullptr) {
          if (iter->second->leaves_[i]->plane_ptr_->is_update) {
            ...
            Eigen::Vector3d plane_cov = iter->second->leaves_[i]
